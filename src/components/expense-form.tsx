...
export function ExpenseForm({
	...
	useEffect(() => {
		...
		if (formData.splitType === "equal") {
			const includedMembers = currentSplits.filter(
				(split) => split.included
			);
			...
			const totalShares = currentSplits.reduce(
		...
		setFormData((prev) => {
			...
			if (prev.splitType === "equal") {
				...
				if (!isNaN(totalAmount)) {
					const includedCount = updated.filter(
						(split) => split.included
					).length;
					if (includedCount > 0) {
	...
	const validateSplits = () => {
		...
		const splitSum = formData.memberSplits.reduce(
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (!validateSplits()) {
			const totalSplit = formData.memberSplits.reduce(
				(sum, split) => sum + (split.amount || 0),
				0
			);
			toast.error(
				`Split amounts (${totalSplit.toFixed(
					2
				)}) don't equal expense amount (${amountNum.toFixed(2)})`
			);
			return;
		}
